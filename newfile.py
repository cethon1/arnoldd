...
SaiF = types.InlineKeyboardButton(text = "- mY kINg .", url = 'https://t.me/FoRaRnoLd')
channel = types.InlineKeyboardButton(text = "- 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 .", url = 'https://t.me/cethon')
...
def text(message):
 	...
 	if str(id) == own_id:
 		...
 		if "/acc" in acc:
 			...
 			k = ks.replace(" ", "")
 			try:
 				with open('sessions.txt', 'a') as the_combo:
 		...
 		if acc == "• 𝖵𝗂𝖾𝖶 𝖺𝖫𝖫 𝖠𝖼𝖼𝖮𝗎𝗇𝖳 •":
 			try:
 				document = open('sessions.txt', 'rb')
 				m = len(open("sessions.txt").readlines())
